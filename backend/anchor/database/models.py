...
class Trade(Base):
    ...
    close_source:            Mapped[str | None]      = mapped_column(String(32))
    broker_verified:         Mapped[bool]            = mapped_column(Boolean, nullable=False, default=False)
...
class ManualTradeJournal(Base):
    ...
    action_key:             Mapped[str]              = mapped_column(String(160), nullable=False, unique=True)
    ...
    anchor_generated_at:    Mapped[datetime | None]  = mapped_column(DateTime(timezone=True))
    anchor_reference_price: Mapped[Decimal | None]   = mapped_column(Numeric(18, 6))
    anchor_stop_price:      Mapped[Decimal | None]   = mapped_column(Numeric(18, 6))
    anchor_entry_note:      Mapped[str | None]       = mapped_column(Text)
